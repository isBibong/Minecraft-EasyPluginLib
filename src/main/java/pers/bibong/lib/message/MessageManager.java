...
public final class MessageManager {
    ...
    public static @NotNull String toColorMessage (final @NotNull String identifier, final @NotNull String message) {
    ...
    public static String toRGB (final @NotNull String message) {
        return Pattern.compile("<#([A-Fa-f0-9]){6}>")
                ...
                .reduce(message,
                        (acc, hexColor) -> acc.replaceAll(hexColor.toString(), hexColor + "$1"),
                        String::concat);
    }
    ...
    public static String toRGBA (final @NotNull String message) {
        ...
        while (matcher.find()) {
            final net.md_5.bungee.api.ChatColor hexColor = net.md_5.bungee.api.ChatColor.of(matcher.group()
                                                                                                    .substring(1,
                                                                                                               matcher.group()
                                                                                                                       .length() - 1));
